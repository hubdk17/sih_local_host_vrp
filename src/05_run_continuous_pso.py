"""
05_run_continuous_pso.py

Runs Continuous Particle Swarm Optimization (Continuous PSO) using Random Key (SPV) encoding
for Vehicle Routing Problem (VRP) over Chandigarh's OpenStreetMap road network graph.
Generates metrics report, Continuous PSO convergence plot, and final VRP route map.
"""
...
from src.continuous_pso_optimizer import ContinuousPSO_VRP
...
CONVERGENCE_PATH = os.path.join(BASE_DIR, "outputs", "continuous_pso_convergence.png")
MAP_VIZ_PATH = os.path.join(BASE_DIR, "outputs", "continuous_pso_vrp_solution_visualization.png")


def plot_convergence(history_time: list, history_distance: list, save_path: str):
    """Plot Continuous PSO convergence curves for travel time and distance."""
    ...
    # Plot Travel Time
    ax1.plot(iters, history_time, color="#27ae60", linewidth=2.0, label="Best Travel Time")
    ax1.set_title("Continuous PSO: Travel Time Convergence", fontsize=12, fontweight="bold")
    ...
    # Plot Distance
    ax2.plot(iters, history_distance, color="#d35400", linewidth=2.0, label="Best Distance")
    ax2.set_title("Continuous PSO: Distance Convergence", fontsize=12, fontweight="bold")
    ...
    print(f"[VISUALIZATION] Continuous PSO Convergence plot saved to: {save_path}")
...
def visualize_pso_routes(instance: VRPInstance, evaluation: dict, save_path: str):
    """Plot optimized Continuous PSO routes on the Chandigarh road network graph."""
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    G = instance.graph
    ...
    for i, (node_path, edge_path) in enumerate(zip(evaluation["vehicle_full_node_paths"], evaluation["vehicle_full_edge_paths"])):
        ...
        ax.plot(
            ...
            linewidth=2.5,
            alpha=0.85,
            label=f"Vehicle {i+1} Continuous PSO Route",
            zorder=3
        )
    ...
    ax.set_title(
        "Continuous PSO-Optimized VRP Routes (Chandigarh Road Graph)",
        fontsize=13,
        fontweight="bold",
        pad=12
    )
    ...
    fig.tight_layout()
    fig.savefig(save_path, dpi=300, bbox_inches="tight")
    print(f"[VISUALIZATION] Optimized Continuous PSO route map saved to: {save_path}")
...
def main():
    print("==================================================")
    print("   MILESTONE 5: CONTINUOUS PSO VRP OPTIMIZER      ")
    print("==================================================\n")
    ...
    # 2. Initialize and Run Continuous PSO Optimizer
    pso = ContinuousPSO_VRP(
        instance=instance,
        num_particles=100,
        iterations=1000,
        w_max=0.9,
    ...
    print("==================================================")
    print("        CONTINUOUS PSO OPTIMIZATION RESULTS       ")
    print("==================================================")
    print(f"Best Decoded Permutation     : {best_individual}")
    ...
    # 5. Plot & Save Continuous PSO Route Map Visualization
    visualize_pso_routes(instance, best_eval, MAP_VIZ_PATH)

    print("\n[SUCCESS] Continuous PSO Optimization completed successfully.")
