"""
ultra_scale_50depot_benchmark.py

Enterprise Mega-Scale Multi-Depot Benchmark:
  - 50 Distribution Hubs / Dark Stores across Delhi-NCR (OpenStreetMap Road Network)
  - 1,000 Delivery Customers with realistic non-uniform demand clusters
  - 90 Delivery Vehicles (Proportional capacity-balanced allocation)
  - Evaluates:
      1. Turing-Enhanced Quantum HQ-GLS (Reaction-Diffusion + Banburismus Deciban Pruning)
      2. Standard Quantum HQ-GLS
      3. Google OR-Tools Guided Local Search (Exact Baseline)
      4. Delta-Well QPSO (Continuous Quantum Swarm)
      5. Classical Genetic Algorithm (Baseline)

Engineered for strict CPU thermal safety:
  - Per-depot local Dijkstra precomputation (<0.03s each)
  - 1.0s time cap per depot on Exact Solver to prevent CPU lockup
  - Clean memory garbage collection between depot batches
"""
...
OUTPUT_DIR = os.path.join(BASE_DIR, "outputs", "ultra_scale_50depot")
...
def run_50depot_benchmark():
    print("=" * 85)
    print(f"  ENTERPRISE MEGA-SCALE VRP BENCHMARK: {NUM_DEPOTS} DEPOTS, {NUM_CUSTOMERS} CUSTOMERS")
    ...
    # 2. Select 50 maximally dispersed Depots via Farthest-First Traversal
    random.seed(SEED)
    np.random.seed(SEED)
    center_node = node_list[len(node_list) // 2]
    all_candidates = [n for n in node_list if n != center_node]

    print(f"[2/4] Synthesizing 50 strategic metropolitan hubs via Farthest-Point Traversal...", flush=True)
    depot_nodes = [center_node]
    sample_pool = random.sample(all_candidates, min(len(all_candidates), 1500))
    ...
    print(f"[3/4] Partitioning 1,000 customers across 50 territorial distribution basins...", flush=True)
    ...
    print(f"  Cluster Sizes: min={min(len(c) for c in cust_clusters.values())}, max={max(len(c) for c in cust_clusters.values())}, avg={NUM_CUSTOMERS/NUM_DEPOTS:.1f} custs/depot")
    print(f"  Fleet Allocation: {sum(vehs_per_depot.values())} vehicles assigned across {NUM_DEPOTS} hubs")

    # 4. Benchmarking Solvers Across All 50 Depots
    print(f"\n[4/4] Executing 5 solvers across 50 hubs (CPU safety caps active)...", flush=True)
    depot_records = []
    ...
    for d in range(NUM_DEPOTS):
        ...
        solver_qp = DeltaWellQPSO(d_time, d_len, d_demands, v_d, VEHICLE_CAPACITY,
                                  depot_coords[d], d_coords, pop_size=20, max_iter=20)
        ...
        solver_ga = ClassicalGABaseline(d_time, d_len, d_demands, v_d, VEHICLE_CAPACITY,
                                       depot_coords[d], d_coords, pop_size=20, generations=20)
        ...
        if (d + 1) % 10 == 0 or d == NUM_DEPOTS - 1:
            print(f"  [Progress] Completed {d + 1:>2}/{NUM_DEPOTS} Hubs | Hub {d+1:>2}: Turing={tur_dist:.1f}km, HQ={hq_dist:.1f}km, OR-Tools={ex_dist:.1f}km", flush=True)
    ...
    # Save CSV & JSON
    csv_path = os.path.join(OUTPUT_DIR, "ultra_scale_50depot_results.csv")
    json_path = os.path.join(OUTPUT_DIR, "ultra_scale_50depot_results.json")
    ...
    # Scorecard
    print("\n" + "=" * 85)
    print(f"  ENTERPRISE 50-DEPOT BENCHMARK COMPLETE (Elapsed: {total_wall_clock:.1f}s)")
    print("=" * 85)
    print(f"  Total Fleet Distance (1,000 Customers across 50 Hubs):")
    ...
    _plot_50depot_dashboard(df, summary_data)
...
def _plot_50depot_dashboard(df, summary):
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 11), facecolor='white')

    # Panel 1: Fleet Distance Comparison
    labels = ['Quantum HQ-GLS\n(Our SOTA)', 'Exact Solver\n(OR-Tools GLS)', 'Turing-Enhanced\nQuantum HQ', 'Delta-Well QPSO\n(Standalone)', 'Classical GA\n(Baseline)']
    distances = [summary["total_hq_dist_km"], summary["total_exact_dist_km"], summary["total_turing_dist_km"], summary["total_qpso_dist_km"], summary["total_ga_dist_km"]]
    ...
    ax1.set_title("Total Fleet Distance (50 Depots, 1,000 Customers) — Lower is Better", fontsize=11, fontweight='bold', color='#0f172a', pad=10)
    ...
    runtimes = [summary["total_hq_time_s"], summary["total_exact_time_s"], summary["total_turing_time_s"], df["qpso_time_s"].sum(), df["ga_time_s"].sum()]
    bars2 = ax2.bar(labels, runtimes, color=colors, width=0.55, edgecolor='#334155', linewidth=1.2)
    ax2.set_title("Total Compute Runtime Across 50 Hubs — Lower is Better", fontsize=11, fontweight='bold', color='#0f172a', pad=10)
    ...
    # Panel 3: Per-Depot Cumulative Distance & Parity Scatter
    depots = df["depot_id"].values
    ax3.scatter(df["exact_dist_km"], df["hq_dist_km"], color='#0284c7', alpha=0.8, s=40, label='Quantum HQ vs Exact (50 Hubs)', zorder=4)
    ...
    ax3.set_title("50-Hub Distance Parity Correlation (y = x)", fontsize=11, fontweight='bold', color='#0f172a', pad=10)
    ...
    # Panel 4: Executive Enterprise 50-Depot Certificate
    ax4.axis('off')
    cert_box = (
        "╔══════════════════════════════════════════════════════════════════════╗\n"
        "║       ENTERPRISE 50-DEPOT BENCHMARK CERTIFICATE OF SCALABILITY        ║\n"
        "╠══════════════════════════════════════════════════════════════════════╣\n"
        f"║ Problem Scale:           50 Depots | 1,000 Customers | 90 Vehicles    ║\n"
    ...
    fig.suptitle("Enterprise Mega-Scale Benchmark: 50 Depots & 1,000 Customers (White Background)",
                 fontsize=14, fontweight='bold', color='#0f172a', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    plot_path = os.path.join(OUTPUT_DIR, "ultra_scale_50depot_benchmark_white.png")
    fig.savefig(plot_path, dpi=300, facecolor='white')
    artifact_plot = os.path.join(ARTIFACT_DIR, "ultra_scale_50depot_benchmark_white.png")
    fig.savefig(artifact_plot, dpi=300, facecolor='white')
    plt.close(fig)
    print(f"  Dashboard saved: {plot_path}")

if __name__ == "__main__":
    run_50depot_benchmark()
