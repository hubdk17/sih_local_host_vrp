"""
delhi_mdvrp_instance.py

Delhi Multi-Depot Vehicle Routing Problem (MDVRP) Instance class.
Manages the Delhi OSM road graph, 4 depots, 50 customers, 10 vehicles,
edge travel time annotations, static road capacities, and pairwise shortest path precomputations.
"""
...
class DelhiMDVRPInstance:
    """
    Manages Delhi Multi-Depot VRP instance parameters and road graph precomputations.
    """
    ...
    def __init__(self, config_path: str, graphml_path: str):
        ...
        # 2. Load GraphML & extract largest SCC
        ...
        # 3. Parse parameters
        self.instance_name = self.config.get("instance_name", "Delhi_MDVRP")
        self.localities_used = self.config.get("localities_used", [])
        ...
        # 4. Annotate edges & precompute shortest paths
        self._annotate_graph_edges()
        self._precompute_shortest_paths()

    def _annotate_graph_edges(self):
        """Annotate each graph edge with travel_time (seconds) and road_capacity."""
    ...
    def _precompute_shortest_paths(self):
        """Precompute shortest paths between all depot and customer location keys."""
        self.shortest_paths = {}
        all_loc_keys = list(self.location_nodes.keys())
        unique_node_ids = list(set(self.location_nodes.values()))

        print(f"[DELHI-MDVRP] Precomputing shortest paths for {len(unique_node_ids)} unique nodes on Delhi graph...")
        ...
        for key1 in all_loc_keys:
            ...
            for key2 in all_loc_keys:
                ...
                if node2 not in paths_dict:
                    raise RuntimeError(f"No path found between {key1} (node {node1}) and {key2} (node {node2})")
        ...
        print(f"[DELHI-MDVRP] Precomputed {len(self.shortest_paths)} shortest path pairs.\n")

    def get_shortest_path(self, loc_key1: Tuple, loc_key2: Tuple) -> dict:
        """Retrieve precomputed shortest path dict for location pair."""
    ...
    def get_vehicle_home_depot_key(self, vehicle_id: int) -> Tuple:
        """Get home depot location key for vehicle_id."""
