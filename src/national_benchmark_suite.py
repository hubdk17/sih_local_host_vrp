"""
national_benchmark_suite.py

Pan-India National VRP Benchmark Suite:
Comparing Heuristic GA (H-GA) vs Dual-Space Quantum Centroid Optimization (DQCO)
across India's top metropolitan cities representing diverse urban topologies:

1. Delhi (NCT)       - Radial / Concentric Megacity
2. Mumbai            - Linear Coastal Peninsula
3. Bengaluru         - Concentric Ring & Radial IT Corridors
4. Kolkata           - Hooghly River-Bisected Linear Corridor
5. Chennai           - Coastal Arc with Radial Spokes

Automated Pipeline:
- Downloads/loads road graph via OSMnx with largest SCC guarantee.
- Automatically locates high-connectivity central logistics hub.
- Runs standardized H-GA vs DQCO benchmark (100 & 500 customers).
- Generates route overlay maps on each city's road network.
- Compiles unified National Scorecard & comparative plots.
"""
...
# City Definitions with precise bounding boxes (west, south, east, north)
CITIES = [
    {
        "key": "delhi",
        "name": "Delhi (NCT)",
        ...
        "cached_path": os.path.join(BASE_DIR, "data", "delhi", "full_delhi_road_network.graphml"),
        "bbox": None  # already cached
    },
    {
        "key": "mumbai",
        "name": "Mumbai",
        ...
        "cached_path": os.path.join(BASE_DIR, "data", "mumbai", "mumbai_road_network.graphml"),
        "bbox": None  # already cached
    },
    {
        "key": "bengaluru",
        ...
        "cached_path": os.path.join(DATA_DIR, "bengaluru", "bengaluru_road_network.graphml"),
        "bbox": (77.48, 12.83, 77.75, 13.14)  # (west, south, east, north)
    },
    {
        "key": "kolkata",
        "name": "Kolkata",
        ...
        "cached_path": os.path.join(DATA_DIR, "kolkata", "kolkata_road_network.graphml"),
        "bbox": (88.25, 22.45, 88.46, 22.65)  # (west, south, east, north)
    },
    {
        "key": "chennai",
        "name": "Chennai",
        ...
        "cached_path": os.path.join(DATA_DIR, "chennai", "chennai_road_network.graphml"),
        "bbox": (80.12, 12.92, 80.32, 13.18)  # (west, south, east, north)
    },
    {
        "key": "hyderabad",
        "name": "Hyderabad",
        "state": "Telangana",
        "topology": "Twin-City Radial / Musi River",
        "cached_path": os.path.join(DATA_DIR, "hyderabad", "hyderabad_road_network.graphml"),
        "bbox": (78.30, 17.30, 78.58, 17.52)  # (west, south, east, north)
    },
    {
        "key": "ahmedabad",
        "name": "Ahmedabad",
        "state": "Gujarat",
        "topology": "River-Bisected Concentric",
        "cached_path": os.path.join(DATA_DIR, "ahmedabad", "ahmedabad_road_network.graphml"),
        "bbox": (72.48, 22.95, 72.68, 23.12)  # (west, south, east, north)
    },
    {
        "key": "pune",
        "name": "Pune",
        "state": "Maharashtra",
        "topology": "Hill-Bounded Confluence Valley",
        "cached_path": os.path.join(DATA_DIR, "pune", "pune_road_network.graphml"),
        "bbox": (73.74, 18.45, 73.98, 18.62)  # (west, south, east, north)
    },
    {
        "key": "chandigarh",
        "name": "Chandigarh",
        "state": "Punjab / UT",
        "topology": "Strict Orthogonal Grid",
        "cached_path": os.path.join(DATA_DIR, "chandigarh", "chandigarh_road_network.graphml"),
        "bbox": (76.70, 30.68, 76.85, 30.78)  # (west, south, east, north)
    },
    {
        "key": "jaipur",
        "name": "Jaipur",
        "state": "Rajasthan",
        "topology": "Historic Grid + Radial Sprawl",
        "cached_path": os.path.join(DATA_DIR, "jaipur", "jaipur_road_network.graphml"),
        "bbox": (75.72, 26.82, 75.90, 26.98)  # (west, south, east, north)
    }
]
...
# Graph Management
# -----------------------------------------------------------------------------
def get_or_download_city_graph(city_info):
    path = city_info["cached_path"]
    if os.path.exists(path):
        print(f"[CACHE] Loading {city_info['name']} graph: {path}...", flush=True)
        G = ox.load_graphml(path)
        print(f"[CACHE] Loaded: {len(G.nodes)} nodes, {len(G.edges)} edges", flush=True)
        return G

    os.makedirs(os.path.dirname(path), exist_ok=True)
    bbox = city_info["bbox"]
    print(f"[DOWNLOAD] Querying OSMnx for {city_info['name']} (bbox={bbox})...", flush=True)
    custom_filter = '["highway"~"motorway|trunk|primary|secondary|tertiary|unclassified"]'

    G_raw = ox.graph_from_bbox(bbox=bbox, custom_filter=custom_filter, simplify=True)
    print(f"[RAW] Downloaded: {len(G_raw.nodes)} nodes, {len(G_raw.edges)} edges", flush=True)

    print(f"[PROCESS] Extracting largest strongly connected component (SCC)...", flush=True)
    ...
    print(f"[SAVE] Cached {city_info['name']} road network to: {path}", flush=True)
    return G
...
def precompute_matrices(node_list, node_to_idx, time_adj, length_adj, depot_node, cust_nodes):
    ...
    return time_mat, dist_mat, time.time() - t0
...
# Route Plotting
# -----------------------------------------------------------------------------
def plot_city_routes(G, depot_coord, cust_coords, routes, total_dist, city_name, scale_name, output_path):
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.set_title(f"{city_name} Road Network ({scale_name}) - DQCO Vehicle Routes\nTotal Fleet Distance: {total_dist:.2f} km (100% Feasible)",
    ...
    ax.scatter(depot_coord[1], depot_coord[0], color='#f1c40f', s=250, marker='*', edgecolor='black', linewidth=1.8,
               label="Central Logistics Depot", zorder=6)
    ...
    for v, route in enumerate(routes):
        ...
        ax.scatter(xs[1:-1], ys[1:-1], color=color, s=26, edgecolor='black', linewidth=0.5, zorder=4)
    ...
    plt.close()
    print(f"[SAVE] Route map saved: {output_path}", flush=True)
...
def main():
    ...
    print("=" * 95, flush=True)
    print("      PAN-INDIA NATIONAL VRP BENCHMARK SUITE: H-GA vs DUAL-SPACE QUANTUM (DQCO)", flush=True)
    print("      Evaluating across Diverse Indian Urban Topologies", flush=True)
    print("=" * 95, flush=True)
    ...
    for city_info in CITIES:
        ...
        print("\n" + "=" * 95, flush=True)
        print(f"  CITY: {city_name.upper()} ({city_info['state']}) | Topology: {topology}", flush=True)
        print("=" * 95, flush=True)

        city_output_dir = os.path.join(OUTPUT_DIR, city_key)
        os.makedirs(city_output_dir, exist_ok=True)

        G = get_or_download_city_graph(city_info)
        ...
        print(f"[DEPOT] Central Logistics Hub for {city_name}: Node {depot_node} at ({depot_coord[0]:.4f} N, {depot_coord[1]:.4f} E)", flush=True)
        ...
        for scale in BENCHMARK_SCALES:
            ...
            print(f"\n  --- Testing {city_name} @ {num_cust} Customers / {num_veh} Vehicles ---", flush=True)
            ...
            print(f"  --> H-GA: {hga_dist:.2f} km | Violations: {hga_viol} | Time: {hga_runtime:.2f}s", flush=True)
            print(f"  --> DQCO: {dqco_dist:.2f} km | Violations: {dqco_viol} | Time: {dqco_runtime:.2f}s", flush=True)
            print(f"  ==> WINNER: {winner} (Margin: {gain_pct:+.2f}%)", flush=True)

            # Save route map for 100 customers
            if num_cust == 100:
                route_map_path = os.path.join(city_output_dir, f"{city_key}_100_cust_routes_map.png")
                plot_city_routes(G, depot_coord, cust_coords, routes, dqco_dist, city_name, scale_name, route_map_path)
            ...
            temp_df = pd.DataFrame(national_results)
            csv_path = os.path.join(OUTPUT_DIR, "national_scorecard.csv")
            temp_df.to_csv(csv_path, index=False)

    df = pd.DataFrame(national_results)
    csv_path = os.path.join(OUTPUT_DIR, "national_scorecard.csv")
    df.to_csv(csv_path, index=False)
    print(f"\n[SAVE] Final National Scorecard saved to: {csv_path}", flush=True)

    # National Visualization (Comparative Bar Chart across all 10 Top Metropolises)
    fig, axes = plt.subplots(1, 2, figsize=(22, 8))
    fig.suptitle("Top-10 Indian Metropolises VRP Benchmark: Heuristic GA vs Dual-Space Quantum (DQCO)", fontsize=15, fontweight="bold")

    # Filter for 100 customers
    df_100 = df[df["scale"] == "100_cust"].copy()
    cities = df_100["city_name"]
    x = np.arange(len(cities))
    w = 0.35

    ax1 = axes[0]
    ax1.bar(x - w/2, df_100["hga_distance_km"], width=w, label="Heuristic GA (H-GA)", color="#3498db")
    ax1.bar(x + w/2, df_100["dqco_distance_km"], width=w, label="Dual Quantum Centroids (DQCO)", color="#2ecc71")
    ax1.set_xticks(x)
    ax1.set_xticklabels([f"{c}\n({t})" for c, t in zip(cities, df_100["topology"])], fontsize=8.5, rotation=25, ha="right")
    ax1.set_ylabel("Total Fleet Distance (km)", fontsize=11)
    ax1.set_title("100 Customers / 10 Vehicles Fleet Distance across Top 10 Indian Cities", fontsize=12, fontweight="bold")
    ax1.legend(fontsize=10)
    ax1.grid(True, linestyle="--", alpha=0.5)

    # Gain percentage bar chart
    ax2 = axes[1]
    gains = df_100["dqco_gain_pct"]
    bar_colors = ["#2ecc71" if g > 0 else "#e74c3c" for g in gains]
    bars = ax2.bar(x, gains, width=0.5, color=bar_colors)
    ax2.axhline(0, color="gray", linewidth=1)
    ax2.set_xticks(x)
    ax2.set_xticklabels(cities, fontsize=9.5, fontweight="bold", rotation=25, ha="right")
    ax2.set_ylabel("DQCO Advantage Margin (%)", fontsize=11)
    ax2.set_title("DQCO Distance Reduction vs Classical GA (100 Customers)", fontsize=12, fontweight="bold")
    ax2.grid(True, linestyle="--", alpha=0.5)

    for bar in bars:
        yval = bar.get_height()
        va = "bottom" if yval >= 0 else "top"
        ax2.text(bar.get_x() + bar.get_width()/2.0, yval, f"{yval:+.1f}%", ha="center", va=va, fontsize=9, fontweight="bold")

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    comp_plot_path = os.path.join(OUTPUT_DIR, "national_performance_comparison.png")
    plt.savefig(comp_plot_path, dpi=300)
    plt.close()
    print(f"[VISUALIZATION] Top-10 National comparative plot saved: {comp_plot_path}", flush=True)

    print("=" * 95, flush=True)
    print("      PAN-INDIA NATIONAL BENCHMARK SUITE COMPLETE!", flush=True)
    print("=" * 95, flush=True)
